...
const WaveShader = () => {
  ...
  useEffect(() => {
    shaderRef.current?.uniforms.uLightCColor.value.set(colorCLight)
  }, [colorCLight])

  useEffect(() => {
    config.lights.c.spherical.phi = phiCLight
    shaderRef.current?.uniforms.uLightCPosition.value.setFromSpherical(config.lights.c.spherical)
  }, [phiCLight])

  useEffect(() => {
    config.lights.c.spherical.theta = thetaCLight
    shaderRef.current?.uniforms.uLightCPosition.value.setFromSpherical(config.lights.c.spherical)
  }, [thetaCLight])
  ...
  return (
    <mesh ref={sphereRef}>
      ...
      <waveShaderMaterial
        ...
        uLightCIntensity={intensityCLight}
