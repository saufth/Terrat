export const vertexShader = `
  ...
  void main() {
    ...
    vec3 displcedPositionA = getDisplacedPosition(positionA).xyz;

    vec3 positionB = position + biTangent.xyz * distanceB;
    vec3 displcedPositionB = getDisplacedPosition(positionB).xyz;
    ...
    float lightCIntensity = max(0.0, -dot(computedNormal.xyz, normalize(-uLightCPosition))) * uLightCIntensity;

    vec3 color = vec3(0.0);
    color = mix(color, uLightAColor, lightAIntensity * fresnel);
    color = mix(color, uLightBColor, lightBIntensity * fresnel);
    color = mix(color, uLightCColor, lightCIntensity * fresnel);
